...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'dqac') {
    header("Location: login.php");
    exit();
}

$sql = "SELECT COUNT(id) as count FROM applications WHERE status = 'Pending DQAC Approval'";
$pending_count = $conn->query($sql)->fetch_assoc()['count'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="main-layout">
    <aside class="sidebar">
        <div class="sidebar-header"><h3>DQAC Portal</h3></div>
        <nav class="sidebar-nav">
            <ul>
                <li><a href="dqac_dashboard.php" class="active"><i class="fa-solid fa-table-columns"></i> Dashboard</a></li>
            </ul>
        </nav>
    </aside>
        ...
        <main class="content-area">
            <h1 class="page-title">Dashboard</h1>
             <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-icon" style="background-color: #e0f2fe;"><i class="fa-solid fa-inbox" style="color: #0369a1;"></i></div>
                    <div class="stat-info">
                        <span class="stat-number"><?php echo $pending_count; ?></span>
                        <span class="stat-label">Applications Awaiting DQAC Approval</span>
                    </div>
                </div>
            </div>
            <div class="card">
                ...
                <table class="data-table">
                    <thead>
                        <tr><th>Student Name</th><th>Department</th><th>Company</th><th>Submitted On</th><th>Action</th></tr>
                    </thead>
                    <tbody>
                        <?php
                        $sql = "SELECT a.id, a.company_name, a.submitted_at, u.full_name, u.department
                                FROM applications a 
                                JOIN users u ON a.student_id = u.id 
                                WHERE a.status = 'Pending DQAC Approval'
                        ...
                        ?>
                        <tr><td colspan="5" style="text-align:center;">No applications are awaiting DQAC approval.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
